...
class ACLASS(Enum):
    ...
    @classmethod
    def construct_class_bits (cls, aclass_: "ACLASS" ) -> int:
        """
        ...
        """
        return aclass_.value << 0 # handle bitfield at the 0-3
...
class a_construction:
    ...
    def __init__(self, aname: ANAME, atype: ATYPE = ATYPE.A, aclass: ACLASS = ACLASS.IN):
        """
        Initialize the Answer Section with ANAME, ATYPE, and ACLASS.

        :param qname: An instance of NAME for domain name handling.
        :param qtype: Answer type (e.g., A, AAAA, MX).
        :param qclass: Answer class (e.g., IN).
        """
        self.aname = aname
        self.qtype = atype
        self.qclass = aclass
    ...
    def construct(self) -> bytes:
        ...
        aname_bytes = self.aname.encode()
        # Convert QTYPE and QCLASS to their binary representations
        atype_bytes = self.atype.value.to_bytes(2, "big")
        aclass_bytes = self.aclass.value.to_bytes(2, "big")
        # Combine all parts into the full Question Section
        return aname_bytes + atype_bytes + aclass_bytes
